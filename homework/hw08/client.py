...
@log
def create_message(from_user, to_user, client_message):
    message = {
        ACTION: MESSAGE,
        TIME: time.time(),
        FROM: {
...
def users_communication(sock, client_account_name):
    while True:
        action = input("Введите команду: ")
        if action == 'message':
            to_user = input('Введите получателя сообщения: ')
            client_message = input("Введите сообщение для отправки: ")
            send_message(sock, create_message(client_account_name, to_user, client_message))
...
@log
def get_message_from_server(sock, client_account_name):
    while True:
        ...
        if ACTION in response and FROM in response and MESSAGE_CLIENT in response \
                ...
                TO in response and response[TO][ACCOUNT_NAME] == client_account_name:
...
def make_sock_send_msg_get_answer():
    ...
    try:
        ...
        getting_thread = threading.Thread(target=get_message_from_server, args=(sock, client_account_name))
        getting_thread.daemon = True
        getting_thread.start()

        sending_thread = threading.Thread(target=users_communication, args=(sock, client_account_name))
